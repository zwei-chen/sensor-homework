...
FrontEndFlow::FrontEndFlow(ros::NodeHandle& nh) {
    ...
    front_end_ptr_ = std::make_shared<FrontEnd>();  // 初始化前端里程计
...
bool FrontEndFlow::UpdateLaserOdometry() {
    ...
    // 初始化时以gnss信号作为雷达里程计的位姿
    if (!front_end_pose_inited) {
        front_end_pose_inited = true;
        front_end_ptr_->SetInitPose(gnss_odometry_);
    }

    laser_odometry_ = Eigen::Matrix4f::Identity();  // 当前帧的雷达位姿，在进行计算时先初始化
    return front_end_ptr_->Update(current_cloud_data_, laser_odometry_);    // 进行雷达里程计的更新
}

bool FrontEndFlow::PublishData() {
    lidar_to_map_tf_pub_ptr_->SendTransform(laser_odometry_, current_cloud_data_.time);
    laser_odom_pub_ptr_->Publish(laser_odometry_);      // 发布雷达里程计
    gnss_pub_ptr_->Publish(gnss_odometry_);             // 发布gnss里程计

    front_end_ptr_->GetCurrentScan(current_scan_ptr_);  // 发布当前扫描
    cloud_pub_ptr_->Publish(current_scan_ptr_);         // 发布当前帧点云

    if (front_end_ptr_->GetNewLocalMap(local_map_ptr_)) // 发布局部地图
