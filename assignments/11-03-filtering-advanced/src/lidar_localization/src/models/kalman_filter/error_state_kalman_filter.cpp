...
ErrorStateKalmanFilter::ErrorStateKalmanFilter(const YAML::Node &node) {
  ...
  RPosiVel_.block<3, 3>(0, 0) = COV.MEASUREMENT.POSE.POSI * Eigen::Matrix3d::Identity();
  RPosiVel_.block<3, 3>(3, 3) = COV.MEASUREMENT.POSE.ORI * Eigen::Matrix3d::Identity();
  RPosiVel_.block<3, 3>(6, 6) = COV.MEASUREMENT.VEL*Eigen::Matrix3d::Identity();
  ...
  GPoseVelCons_.block<7, kDimState>(0, 0) = GPoseVel_.block<7, kDimState>(0, 0);
  GPoseVelCons_.block<1, kDimState>(7, 0) = GPoseVel_.block<1, kDimState>(8, 0);
  ...
  GPosiVel_.block<3, 3>(0, kIndexErrorPos) = Eigen::Matrix3d::Identity();
  GPosiVel_.block<3, 3>(3, kIndexErrorOri) = Eigen::Matrix3d::Identity();
  CPosiVel_.block<3, 3>(0, 0) = Eigen::Matrix3d::Identity();
  CPosiVel_.block<3, 3>(3, 3) = Eigen::Matrix3d::Identity();
...
void ErrorStateKalmanFilter::CorrectErrorEstimationPoseVel(
  ...
  // set measurement:
    ...
    YPoseVelCons_.block<3, 1>(0, 0) = dp;      
    YPoseVelCons_.block<3, 1>(3, 0) = dtheta;  
    YPoseVelCons_.block<2, 1>(6, 0) = dv.block<2,1>(1,0);      
    Y = YPoseVelCons_;                         
...
void ErrorStateKalmanFilter::CorrectErrorEstimationPosiVel(
    ...
    YPosiVel_.block<3, 1>(3, 0) = dtheta;  
    YPosiVel_.block<3, 1>(6, 0) = dv;      
    Y = YPosiVel_;                         
...
void ErrorStateKalmanFilter::CorrectErrorEstimation(
  ...
  case MeasurementType::POSE_VEL:
    ...
    CorrectErrorEstimationPoseVel(
      measurement.T_nb,measurement.v_b,
