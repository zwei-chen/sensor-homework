...
bool OdoPreIntegrator::Update(const PoseData &odom_data) {
    if ( odo_data_buff_.front().time < odom_data.time ) {
      // set buffer:
      VelocityData tmp_v;
      tmp_v.time = odom_data.time;
      tmp_v.angular_velocity.x = odom_data.vel.w.x();
      tmp_v.angular_velocity.y = odom_data.vel.w.y();
      tmp_v.angular_velocity.z = odom_data.vel.w.z();
      tmp_v.linear_velocity.x = odom_data.vel.v.x();
      tmp_v.linear_velocity.y = odom_data.vel.v.y();
      tmp_v.linear_velocity.z = odom_data.vel.v.z();

        odo_data_buff_.push_back(tmp_v);
