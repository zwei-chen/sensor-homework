...
class DataPretreatFlow {
  ...
  private:
    // subscriber
    std::shared_ptr<CloudSubscriber> cloud_sub_ptr_;        // velodyne激光雷达测量值的接受者
    std::shared_ptr<IMUSubscriber> imu_sub_ptr_;            // OXTS惯导imu的接受者
    std::shared_ptr<VelocitySubscriber> velocity_sub_ptr_;  // OXTS惯导速度的接受者
    std::shared_ptr<GNSSSubscriber> gnss_sub_ptr_;          // OXTS惯导Gnss数据的接受者
