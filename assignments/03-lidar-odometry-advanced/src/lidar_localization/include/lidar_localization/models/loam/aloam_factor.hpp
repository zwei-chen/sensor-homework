...
/**
 * @brief针对ceres中，设置SE3的迭代更新形式，此处参考floam
 */ 
...
/**
 * @brief 线特征残差解析式求解，此处参考floam
 * @param cur_point_ pi
 * @param last_point_a_ pa
 * @param last_point_b_ pb
 */ 
class EdgeAnalyticCostFunction : public ceres::SizedCostFunction<1, 7> {
	public:
		...
		virtual bool Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
		{
            ...
            Eigen::Vector3d lp;
            lp = q_last_curr * curr_point + t_last_curr; // pi~
            Eigen::Vector3d nu = (lp - last_point_a).cross(lp - last_point_b);	// (pi~-pa)x(pi~-pb)
            Eigen::Vector3d de = last_point_a - last_point_b;	// pa-pb
            
            Eigen::Vector3d residualsVector;
            residualsVector << nu.x() / de.norm(), nu.y() / de.norm(), nu.z() / de.norm();
            residuals[0] = residualsVector.norm();	// 残差
            ...
            if(jacobians != NULL)
            {
                if(jacobians[0] != NULL)
                {
                    Eigen::Matrix3d skew_lp = skew(lp);			// 叉乘算子
                    Eigen::Matrix<double, 3, 6> dp_by_so3;
                    dp_by_so3.block<3,3>(0,0) = -skew_lp;		// 对应公式第二项的-(Rpi+t)^
                    (dp_by_so3.block<3,3>(0, 3)).setIdentity();	// 对应公式第二项的I
                    Eigen::Map<Eigen::Matrix<double, 1, 7, Eigen::RowMajor> > J_se3(jacobians[0]);
                    J_se3.setZero();
                    Eigen::Vector3d re = last_point_b - last_point_a;
                    Eigen::Matrix3d skew_re = skew(re);

                    J_se3.block<1,6>(0,0) = (residualsVector/residuals[0]).transpose()*skew_re * dp_by_so3/de.norm();	// 与floam相比，(residualsVector/residuals[0])等同于增加了A/|A|（约去了常数项），A的含义见公式推导，
...
/**
 * @brief 面特征残差解析式求解，此处参考floam，由于两者求导无区别，故形式和floam一致
 * @param cur_point_ pi
 * @param last_point_j_ pj
 * @param last_point_l pl
 * @param last_point_m pm
 */ 
class SurfNormAnalyticCostFunction : public ceres::SizedCostFunction<1, 7> {
	public:
		...
		virtual bool Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
		{
            ...
            residuals[0] = plane_unit_norm.dot(point_w) + negative_OA_dot_norm;	// 残差

            if(jacobians != NULL)
            {
                if(jacobians[0] != NULL)
                {
                    Eigen::Matrix3d skew_point_w = skew(point_w);	// 同线特征

                    Eigen::Matrix<double, 3, 6> dp_by_so3;
                    dp_by_so3.block<3,3>(0,0) = -skew_point_w;		// 同线特征
                    (dp_by_so3.block<3,3>(0, 3)).setIdentity();		// 同线特征
                    Eigen::Map<Eigen::Matrix<double, 1, 7, Eigen::RowMajor> > J_se3(jacobians[0]);
                    J_se3.setZero();
